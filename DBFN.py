...
class UNet(nn.Module):
    def __init__(self, dim=128):
        super().__init__()
        self.encoder = SwinTransformer(in_chans=4,depths=[2, 2, 6, 2], num_heads=[4, 8, 16, 32], drop_path_rate=0.5,
                                       embed_dim=128)
        self.encoder2 = SwinTransformer(in_chans=1,depths=[2, 2, 6, 2], num_heads=[4, 8, 16, 32], drop_path_rate=0.5,
                                       embed_dim=128)
        self.layer1 = Swin_Decoder(16 * dim, 2, 8)
        self.layer2 = Swin_Decoder(8 * dim, 2, 4)
        self.layer3 = Swin_Decoder(4 * dim, 2, 2)
        self.up3=up_conv1(dim*2,4)
        dim_l = dim
        self.m2 = nn.Upsample(scale_factor=4)
        self.cross_att_1 = Cross_Att(dim_l * 1, dim_l * 1)
        self.cross_att_2 = Cross_Att(dim_l * 2, dim_l * 2)
        self.cross_att_3 = Cross_Att(dim_l * 4, dim_l * 4)
        self.cross_att_4 = Cross_Att(dim_l * 8, dim_l * 8)

    def forward(self, x,y):
        out = self.encoder(x)
        out2 = self.encoder2(y)
        ...
        e1 = torch.cat([r1, self.m2(e1)], 1)
        e2 = torch.cat([r2, self.m2(e2)], 1)
        e3 = torch.cat([r3, self.m2(e3)], 1)
        e4 = torch.cat([r4, self.m2(e4)], 1)
